  ...
  @objc func addToFavoritesPressed(sender: UIButton) {
    ...
    PersistenceManager.shared.checkIfTeamIsInFavorites(teamId: activeArray[teamIndex].id) { isInFavorites in
      switch isInFavorites {
        case true:
          PersistenceManager.shared.updateWith(favorite: activeArray[teamIndex], actionType: .remove) { _ in }
          sender.setImage(UIImage(systemName: "heart"), for: .normal)
        case false:
          sender.setImage(UIImage(systemName: "heart.fill"), for: .normal)
          PersistenceManager.shared.updateWith(favorite: activeArray[teamIndex], actionType: .add) { _ in }
      }
    }
    tableView.reloadData()
  }
