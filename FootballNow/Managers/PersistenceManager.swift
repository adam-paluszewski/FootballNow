...
//    Retrieve with generics
//    func retrieve<T: Codable>(for key: Keys, completionHandler: (Result<[T],FNError>) -> Void) {
//        guard let lastSearchedData = defaults.object(forKey: key.rawValue) as? Data else {
//            completionHandler(.success([]))
//            return
//        }
//
//        do {
//            let decoder = JSONDecoder()
//            let lastSearched = try decoder.decode([T].self, from: lastSearchedData)
//            completionHandler(.success(lastSearched))
//        }
//        catch {
//                completionHandler(.failure(.invalidData))
